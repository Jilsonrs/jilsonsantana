...
- **Leitura pública filtra por idioma do mesmo jeito que filtra por status** — catálogo, busca, lista de trilhas, `sitemap.xml`. Quem entra em inglês vê só conteúdo em inglês, **inclusive enquanto não existir nenhum** (o catálogo vazio precisa de estado próprio). **O gate de acesso NÃO lê idioma:** `temAcessoAtivo()` fica intocado.
