...
- **Leitura pública filtra por idioma do mesmo jeito que filtra por status** — catálogo, busca, lista de trilhas, `sitemap.xml`. Quem entra em inglês vê só conteúdo em inglês. **Curso publicado com zero aulas é caso real** (os primeiros cursos em inglês nascem assim) e precisa de estado próprio, nunca "0 módulos · 0 aulas".
...
- **No máximo 2 idiomas** *(decisão do operador)*: o enum tem dois valores, e um terceiro é decisão nova dele, não "mais um valor". Endereços em inglês usam segmentos em inglês (`/en/courses`).
