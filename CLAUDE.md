...
- **Ainda falta no bloco Gates: rate-limit de login.** Bloco próprio; toca `server/src/lib/auth.ts`, logo **dispara o gate do context7** (`/better-auth/better-auth`). **O passo 1 é MEDIR a borda, nunca codar** — o protocolo executável (rota temporária, os três testes, o critério de aprovação) é checkbox em `implementation-plan.md` → Fase 3, Bloco 0. Duas armadilhas ficam aqui porque são o que faz alguém "simplificar" errado:
  ...
  2. **A 1.7.5 NÃO é mais burlável por header forjado — e o risco MUDOU DE FORMA, não desapareceu.** `[FATO — medido em Set 2026 no código da 1.7.5: `@better-auth/core/dist/utils/ip.mjs`, `getIPFromHeader`.]` Sem `trustedProxies` configurado, **cadeia com mais de um elemento devolve `null`** (`if (forwardedIps.length !== 1) return null`). Com `trustedProxies`, ele caminha a cadeia **da direita para a esquerda**, pula os saltos confiáveis e pega o primeiro não confiável — que é o cliente de verdade.
     **O que acontece quando o IP não resolve** `[FATO — `dist/api/rate-limiter/index.mjs:239-245`]`: o limitador cai num **BALDE ÚNICO por rota**, compartilhado por todo mundo. Ou seja: o desvio de brute-force fechou, e no lugar dele nasceu uma **negação de serviço** — um atacante sozinho esgota o balde e tranca o login de **todos**.
     **E ele AVISA NO LOG**, uma vez: *"Rate limiting could not determine a client IP and is falling back to a single shared per-path bucket"*. **Isto substitui a rota temporária de diagnóstico do Passo 1:** a presença ou ausência dessa linha nos logs da Railway responde, sem código nenhum, se o IP está resolvendo.
     *(Histórico, para ninguém "consertar" de volta: na 1.6.20 era o oposto — `value.split(",")[0]`, o primeiro elemento vencia, e um header forjado escolhia o balde. A atualização para a 1.7.5, em Set 2026, foi decisão do operador justamente por isso.)*
  3. **`customRules` EXISTE** `[FATO — `dist/api/rate-limiter/index.mjs`]`: dá para apertar `/sign-in/email` sozinho (ex.: 3 tentativas em 10 s) sem mexer no resto. **Mas NÃO aperte antes de resolver o IP, e o motivo inverteu com a 1.7.5:** se o balde estiver compartilhado, um limite apertado vira a própria negação de serviço — três tentativas erradas de qualquer pessoa trancam o login de todos, inclusive o do único admin. Apertar é o passo DEPOIS de o IP resolver, nunca antes.
