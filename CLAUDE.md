...
- **O `globalSetup` do E2E carrega a MESMA trava de host local** da suíte de servidor (`ALLOWED_TEST_HOSTS`, importada — **não** copiada: duas cópias divergem, e a que diverge é a que ninguém está olhando), e **roda o mesmo seed** — não um segundo seed "parecido". Duas razões, ambas já materializadas neste repo: sem a trava, `playwright test` aponta para o que o `.env` disser; com seed duplicado, o E2E passa a testar um mundo que o servidor não tem, e a divergência aparece meses depois como falha intermitente. *(PENDENTE — é o Bloco T1; hoje `e2e/playwright.config.ts` ainda lê `../server/.env`.)*
