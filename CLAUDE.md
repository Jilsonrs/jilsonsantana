...
- **Dunning is product policy, not our code.** The ruler (D0 → retries → cutoff) stays OUR decision — above all **access is KEPT during the retry window** (`past_due`), because involuntary churn is the biggest lever (strategy.md). What changed is who executes it: Stripe Smart Retries + recovery automations, not a pg-boss scheduler of ours.
