...
- `npm run typecheck` · `npm run build` · `npm run test` — os três gates da raiz. **`test` cobre `client` (Vitest + RTL) E `server` (Vitest + supertest)** desde Ago 2026. Playwright **não** entra neste script — precisa de servidor de pé.
  **A suíte do `server` fala com um banco REAL** (`mvaobzypsiuhqzipcelw`) e o `globalSetup` dá `prisma migrate reset --force` nele antes de cada execução. Duas consequências: exige `server/.env.test` (senão a trava aborta dizendo o porquê), e **rodar `npm test` APAGA o banco de desenvolvimento**, que hoje é o mesmo projeto. O `build` do `server` usa `tsconfig.build.json`, que exclui `src/test/` — código de teste não vai para a imagem de produção.
...
- **Banco de TESTE = um SEGUNDO projeto Supabase** (decidido Ago 2026), **não** Postgres local em Docker — uma camada a menos de infra pra operador solo. **EXISTE desde Ago 2026:** ref `mvaobzypsiuhqzipcelw` ("Jilson Santana Test", us-east-1). O antigo `[PENDENTE DE VERIFICAÇÃO]` sobre o tier grátis está **resolvido**: o Free permite 2 projetos ativos. O mesmo projeto serve **dev E teste** (ver *Separação de ambientes* abaixo).
- **TRAVA obrigatória, mesma família do "nunca `migrate dev` contra prod": o setup de teste ABORTA se `DATABASE_URL` não contiver o REF do projeto de teste**, declarado como constante nomeada no `globalSetup`:
  ```ts
  const TEST_DB_REF = "mvaobzypsiuhqzipcelw"; // Jilson Santana Test
  if (!process.env.DATABASE_URL?.includes(TEST_DB_REF)) {
    throw new Error("DATABASE_URL não aponta para o banco de teste — abortando");
  }
  ```
  **Por que existe a trava** (não é paranoia, é o comportamento real da ferramenta): o `globalSetup` do padrão de referência roda `prisma migrate reset --force`, que **dropa todas as tabelas** — apontado pra string errada, apaga a produção **sem pedir confirmação**.
  **Por que o REF e não a substring `_test` — CORREÇÃO de Ago 2026, a regra anterior NÃO FUNCIONAVA:** no Supabase o host é montado a partir do **project ref opaco**, nunca do nome do projeto. Evidência: o projeto chamado "Jilson Santana Website" atende em `db.gaxmbnhwltljlkukdwba.supabase.co` — o nome não aparece em lugar nenhum da string. Logo uma trava contra `_test` **nunca dispararia**, e **trava que nunca dispara é o mesmo defeito do `lint` que mentia** (apagado no Bloco 0 da Fase 3): dá a sensação de proteção sem a proteção. O ref é **único globalmente**, então a trava passa a verificar **IDENTIDADE** em vez de semelhança de texto — um projeto futuro chamado `algo_test` não passa por engano.
- **Separação de ambientes (Ago 2026): dev e teste usam o projeto de TESTE; só o Railway fala com produção.** Existem **TRÊS caminhos** até o banco — Vitest (`globalSetup`), chamada de MCP, e comando digitado à mão — e **só o primeiro tem trava automática.** Os outros dois dependem de disciplina: **declarar contra qual ref se está apontando antes de rodar**. Prisma da raiz do monorepo não resolve o schema (nem o `.env`), então todo comando de migration é digitado de dentro de `server/` — onde o `.env` escolhe o banco.
...
- **Ainda falta no bloco Gates:** rate-limit de login com a borda verificada (bloco próprio — toca auth, exige context7). Claude code review on PRs.
...
> 🚧 **EM VOO — as duas entradas abaixo estão sendo escritas por etapas.** A auditoria de documentação de Ago 2026 foi fatiada em 8 commits para caber em sessões curtas; cada etapa acrescenta suas linhas ao stub certo, no mesmo commit do trabalho. **Consequência esperada, NÃO regressão:** até a última etapa o rodapé tem 5 entradas em vez de 3. A rotação de (8) e (9) para o `decisions-archive.md` acontece na etapa final e fecha o teto. **Não "consertar" isto no meio do caminho.**

*Atualizado Ago 2026 (11) — **convenções da auditoria comparativa (gate de acesso, identificadores públicos, enum, testes, rate-limit).** \[EM VOO\]*
*(a) **A REGRA DO GATE — a decisão que faltava para a Fase 4 começar.** `temAcessoAtivo()` responde SIM quando o **status está vivo** (`active`/`trialing`/`past_due`) **OU** quando o **período já foi pago** (`currentPeriodEnd > agora`), exceto `incomplete`/`incomplete_expired`. **Por que duas metades e não uma tabela status-a-status:** a primeira cobre a assinatura saudável e a janela de Smart Retries — `past_due` **mantém** acesso, porque churn involuntário é a maior alavanca (`strategy.md`); a segunda cobre **cancelamento e pausa com uma regra só** — quem pagou até o dia 15 acessa até o dia 15. Enumerar oito status quebra quando a Stripe inventa o nono. A exceção do `incomplete` existe porque ali o **primeiro** pagamento nunca aconteceu: não há período pago para honrar, e é literalmente o caminho de "acesso sem pagar". **TRAVA registrada junto: `cancel_at_period_end` NÃO é status** — ao cancelar, a Stripe mantém `status: active` com a flag ligada até o fim do período pago, então **o gate lê status e data, nunca a flag**; ler a flag corta o acesso de quem já pagou o mês, e essa pessoa abre chamado no mesmo dia. **Sem gatilho de reabertura — é a especificação do gate, muda só se a Stripe mudar o modelo de status.** Fica um `[VERIFICAR]` mecânico para a Fase 4 (gate do context7 `/websites/stripe`): semântica de `paused`/`pause_collection` e se `currentPeriodEnd` segue populado na pausa — a regra é a especificação, o mapeamento para os campos reais se confirma na hora de codar.*
*(b) **CHURN NÃO APAGA NADA — decisão de produto.** Cancelamento, pausa e inadimplência **removem acesso, nunca dados**: cadastro, `LessonProgress`, `LessonEvent`, certificados e histórico sobrevivem, e quem volta continua de onde parou. Registrado porque é **o oposto do default**: quem implementa "remover acesso" tende a limpar dados junto, e isso destrói em silêncio a maior alavanca de reativação que a escola tem. Não confundir com o `deletedAt` do `User`, que é exclusão a pedido do titular (LGPD) — são coisas diferentes que se parecem no código. **Sem gatilho — arquitetural.***
*(c) **O `try/catch` do e-mail no webhook exige `await`.** `try { enviarEmail() } catch {}` não captura nada: `try/catch` é síncrono e a rejeição de uma promise não aguardada escapa dele, vira *unhandled rejection* e **pode derrubar o processo Node dentro do handler da Stripe**. O typecheck passa, o teste passa, cai em produção. É a falha que motiva o ESLint com `no-floating-promises` na Fase 4 — a regra pega isto automaticamente, e é a única das 2–3 regras iniciais que se justifica sozinha. **Gatilho:** a decisão de adiar o ESLint se reabre se a mesma classe de bug aparecer uma segunda vez antes da Fase 4.*
*(d) **CRITÉRIO DE ENTRADA do `CLAUDE.md`** (seção *Arquivos de memória*): uma linha fica aqui **se, e só se, um agente prestes a escrever código produziria um diff ERRADO sem ela** — numa sessão em que ninguém consultou nada. Tabela de três destinos (decidindo → archive · feature específica → o doc dela ou o checkbox do plano · porquê → fica só se a regra for "simplificada" sem ele) mais a regra de **FORMA**, que é o que segura o tamanho: regra em imperativo, porquê em UMA frase, raciocínio longo no `decisions-archive.md`, **procedimento executável é checkbox no plano, nunca convenção aqui**. **Escrito ANTES das demais etapas da auditoria, de propósito** — a versão original deste plano o colocava por último, o que significa seis blocos de convenção escritos sem o critério valendo, exatamente a inversão que ele existe para impedir. **Medida que motivou a antecipação:** a Etapa 1 sozinha somou 6,5 KB, e a projeção do arquivo com o critério só no fim era ~87 KB contra os 71 KB de partida. **Gatilho de reabertura:** se o `CLAUDE.md` voltar a passar de ~85 KB com o critério em vigor, o critério não está funcionando e o problema é outro — provavelmente escopo, não redação.*
*(e) **IDENTIFICADOR PÚBLICO ≠ CHAVE PRIMÁRIA — e a regra é mais estreita do que a versão genérica "use UUID".** A PK segue inteiro sequencial; ganha uma coluna a mais (`publicId cuid`) **só a entidade cuja URL pública aponta para o registro de UMA PESSOA** — hoje apenas `Certificate`. **Catálogo NÃO entra:** curso, aula e trilha curada usam slug legível, porque são vitrine e slug é melhor para SEO e para o aluno. **O argumento decisivo não é de segurança, é comercial:** `/certificado/3` publica o contador de alunos da escola para qualquer recrutador que repare no número, e a URL do certificado é canal de aquisição por decisão de produto — irreversível depois que alunos publicarem os links. `isPublic` continua ortogonal (decide *se* a rota responde; o `publicId` decide *o que é enumerável*). **Custo hoje zero** — `Certificate` nasce na Fase 6.5, então é convenção que entra em vigor com o model, não migration retroativa; **trocar a PK para UUID foi PROPOSTO e REJEITADO** (não impede falha nomeável e custa caro). **Gatilho:** reabre quando nascer a segunda URL pública ligada a pessoa (trilha compartilhada, perfil), para conferir se a lista ainda cabe numa regra só.*
*(f) **SLUG DE CATÁLOGO É PERMANENTE** — regra que não estava em auditoria nenhuma, veio da decisão de que a página de curso vira vitrine indexável. URL sem extensão (`/curso/<slug>`, nunca `.html`), slug escolhido na criação e **não** acompanha mudança de título. **Por que virou regra e não preferência:** slug indexado que muda perde o ranking acumulado **e** quebra todo link já compartilhado por aluno — e a única saída depois é manter 301 do antigo, o que obriga a guardar os slugs velhos para sempre. É barato agora e caro depois; por isso está escrito antes de existir a primeira página pública. **Sem gatilho — vira restrição permanente no dia em que o Google indexar a primeira URL.***
*(g) **A AUDITORIA COMPARATIVA CONTRA O REPO DE REFERÊNCIA (Mosh/helpdesk) PRODUZIU UM ACHADO DE MÉTODO, não só de conteúdo:** **5 das 8 convenções propostas eram TRANSCRIÇÃO do `CLAUDE.md` dele, e eram FALSAS sobre este repo.** Conferidas uma a uma contra o código: `VITE_API_URL` não existe aqui (o alvo do proxy é fixo) · `erasableSyntaxOnly` não está no `client/tsconfig.json` · o helper de teste chama `renderWithProviders` em `@/test-utils`, não `renderWithQuery` em `@/test/render` · `ErrorAlert`/`ErrorMessage` não existem (são 5 `<p>` inline) · e **nossos testes não mockam `axios`** — mockam `@/lib/api`, que é a nossa fronteira e é o padrão **melhor**, não pior. **Por que isto é entrada de changelog e não uma correção silenciosa:** regra que descreve o repo de outro é a mesma família do `lint` que rodava `tsc --noEmit` e da trava `_test` que nunca disparava — **parece proteção e não descreve nada** —, com o agravante de entrar na lei de build, onde a próxima sessão a lê como verdade e cria um segundo helper ao lado do que já existe. **Regra que fica:** convenção só entra depois de conferida **contra o diff deste repo**; benchmark externo é fonte de *pergunta*, nunca de *resposta*. **Sem gatilho — método.***
*(h) **O que a comparação DE FATO contribuiu, depois da poda:** (i) **a razão do Mosh para o pg-boss deixou de ser DESCONHECIDA e CONFIRMA a remoção** — as filas dele retentam chamada de LLM de terceiro (`retryLimit: 3`, backoff), não o webhook, e isso tem a forma exata do nosso gatilho de volta (JilsonAI 4–5); a referência não discorda, chega mais cedo ao mesmo lugar porque o produto dela é IA assíncrona desde o dia um. (ii) **`User.role` deveria ser Prisma enum e não é** — as outras quatro (`Level`, `ContentStatus`, `Layer`, `PlanItemType`) já são; `role` é `String` livre porque a coluna é do Better Auth. **Decisão: fica como está** (duas vias de escrita, ambas tipadas via `Role`; converter custa mais um item na reaplicação pós-`better-auth generate`), com gatilho na terceira via de escrita. (iii) **O tamanho do arquivo dele é a lição** — ~7,5 KB cobrindo a mesma superfície que os nossos 71 KB, porque as convenções dele são uma linha imperativa sem raciocínio embutido. Foi essa comparação que produziu a regra de **FORMA** do critério de entrada, item (d). **Também registrado: ele NÃO resolve o problema do header confiável** (o rate-limit dele é o mesmo `NODE_ENV=production`), então o Passo 1 da nossa borda é trabalho original — não há referência para copiar.*
*(i) **Convenções reescritas com a razão VERDADEIRA:** TS `enum` proibido porque **emite JavaScript em runtime** (não por `erasableSyntaxOnly`, que não temos) × Prisma `enum` desejável porque **a restrição mora no banco** · dev na mesma origem via proxy do Vite, com o corolário de diagnóstico (`Access-Control-Allow-*` no server = pare antes de adicionar o header) · `renderWithProviders` obrigatório com `QueryClient` novo por teste · mock na nossa fronteira, nunca em `axios`. Corrigidos também os caminhos `core/schemas/` → `core/src/schemas/` e `core/constants/` → `core/src/constants/`, que estavam errados desde sempre.*
