...
- `npm run typecheck` · `npm run build` · `npm run test` — os três gates da raiz. **`test` cobre HOJE só o workspace `client`** (Vitest), porque é o único com runner: o `server` ganha suíte (Vitest + supertest) na Fase 4 e entra aqui como segundo termo. Playwright **não** entra neste script — precisa de servidor + banco de teste.
