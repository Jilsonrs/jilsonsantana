...
- `npx prisma migrate dev --name <snake_case>` — new migration. **Rode de um terminal de verdade:** o comando é interativo e recusa shell de agente (*"environment is non-interactive"*), o que NÃO é defeito do repo.
  **TODA MIGRATION QUE TOCA TABELA CRIADA FORA DO VERSIONAMENTO PRECISA SER CONDICIONAL** (`IF EXISTS`, num bloco `DO $$`). `[FATO — medido em Set 2026, A/B no mesmo shell: a forma incondicional faz o `migrate dev` INTEIRO falhar com P1014; a condicional passa.]` **Causa:** o `migrate dev` valida o histórico replicando-o num **shadow database** — e lá `_prisma_migrations` **não existe**. Foi exatamente o que aconteceu com a migration de RLS de Ago 2026: durante semanas ela impediu qualquer migration nova de ser criada, e o erro apontava para ela, convidando a "correção" errada (apagá-la). Hoje ela é condicional.
  **`migrate deploy` e `migrate reset` NÃO usam shadow** — é por isso que a produção e a suíte de teste nunca sentiram o problema, e é por isso que ele passou despercebido.
  **[FATO — medido] o Prisma 5.22 NÃO reprova checksum de migration já aplicada:** com o arquivo editado e o checksum antigo gravado no banco, `migrate status` e `migrate deploy` passaram. Foi isso que permitiu tornar a migration condicional **sem** `migrate resolve` em nenhum ambiente. *Gatilho: se um Prisma futuro passar a validar checksum, editar migration aplicada volta a exigir intervenção nos dois bancos.*
  **Depois de qualquer migration, prove duas coisas:** `npx prisma migrate diff --from-schema-datasource prisma/schema.prisma --to-schema-datamodel prisma/schema.prisma` (esperado: *No difference detected*) e a **suíte de servidor**, que dá `migrate reset` num banco limpo — é ela que prova que a migration replica **do zero**, que é diferente de "funcionou no meu banco".
- `npx prisma db pull` to reconcile tables created fora das migrations (MCP, painel, psql)
...
- **Cookie de sessão: `httpOnly` + `secure` + `sameSite`.** Sem `httpOnly`, qualquer XSS lê a sessão (por isso esta regra é irmã da proibição de `dangerouslySetInnerHTML` no Client); sem `secure`, ela viaja em claro; sem `sameSite`, CSRF sai de graça. **A convenção fica escrita mesmo onde é default:** ela existe pra ninguém "simplificar" a config depois sem perceber o que desligou.
  **VERIFICADO, e RE-verificado na atualização para a 1.7.5 (Set 2026)** `[FATO — `better-auth/dist/cookies/index.mjs:35,37`]`: `httpOnly: true` e `sameSite: "lax"` continuam default **hardcoded**, não dependem de config. **`secure` NÃO é.** A 1.7.5 ganhou uma checagem de protocolo antes das outras, mas o ramo por `baseURL.startsWith("https://")` **continua existindo** — e como a string é truthy, o fallback por `isProduction` segue inalcançável quando a URL está definida.
  **A conclusão não mudou com a versão nova, e é por isso que a regra fica:** o `secure` continua podendo depender da grafia de uma variável de ambiente. *(Este repo já fixa `advanced: { useSecureCookies: process.env.NODE_ENV === "production" }` em `server/src/lib/auth.ts` — a regra existe para ninguém remover isso achando que o default cobre.)*
...
- **QUANDO HOUVER ALUNO REAL, BRANCH DE `production` PASSA A SER SCHEMA-ONLY.** Um branch normal **copia as linhas** — hoje isso são 2 contas de teste e não custa nada, mas no dia em que houver aluno pagante, `branch production → dev` vira **cópia de dado pessoal para o ambiente menos protegido**, contra a mesma lógica que já proíbe copiar credencial entre ambientes (e contra a minimização de LGPD que fez o `birthday` guardar só dia+mês). O Neon tem **schema-only branch** exatamente para isso: leva a estrutura, não as linhas. *Gatilho, e é verificável: **o primeiro `User` que não seja `jilson@` ou `member@jilsonsantana.com`.** A partir dele, branch de produção para dev/teste é schema-only + seed próprio; branch com dados fica para investigar incidente, com descarte imediato.*
...
- **Todo curso e toda trilha nascem num idioma** (`language`, PT ou EN) — as travas estão em [Idiomas](#idiomas--escola-bilíngue-pt--en-decisão-do-operador-set-2026).
...
**MAS a vitrine é MONTADA DE CAMPOS ESTRUTURADOS, não escrita curso a curso.** `subtitle`, `description`, `learnTags[]`, `requirements[]`, `personas[]`, `highlights[]`, `faq[]` e `camadas[]` já existem e montam a página inteira: **mudou o LAYOUT, não o schema.** **Por que a trava importa:** a razão original do "light by design" era carga de operador solo — catálogo rotativo de até 15 cursos **por idioma** (dois idiomas no máximo) = até 30 páginas de venda para escrever e manter. Preenchendo campos, o custo por curso quase não sobe. **Abrir espaço para copy livre por curso (um campo tipo `salesCopy`) ressuscita exatamente o problema que a decisão original evitava.** *Gatilho de reabertura: dado real de conversão mostrando que a página estruturada converte pior que uma escrita à mão.*
...
- **Textos e ícones das camadas são GLOBAIS, escritos UMA vez (por idioma).** Os **textos** moram no dicionário, em `common.camadas`, nos dois idiomas e **editáveis em Admin → Textos** *(decisão do operador, 24/09/2026)*; o app os lê já com as edições (`useTextosComuns`). O **ícone e a cor** ficam em `core/src/constants/` (`LAYER_CONFIG`). Por curso o operador só **escolhe quais**. É isto que mantém o selo premium sem trabalho de copy por curso. `Course.camadaOverride?` (jsonb) é **exceção, não rotina**: se você sobrescreve em *todo* curso, o texto global está errado — conserte o global.
...
- **`User` fields — keep identity lean, extras optional (Phase 1):**
  ...
  - `preferredLanguage` default `"pt"` — **ativo desde Set 2026** (deixou de ser costura dormente): é o idioma da interface do aluno **logado** (`"pt"` | `"en"`), gravado quando ele troca pelo seletor. Visitante anônimo não tem coluna — o idioma dele é o do **endereço**. Ver *Idiomas*.
...
- **Template:** `server/src/views/home.ts` — é o mock aprovado **transposto**, com as mesmas
  classes. **Mock aprovado se TRANSPÕE, nunca se reinterpreta:** a primeira tentativa reescreveu a
  marcação com classes inventadas e metade da página ficou sem estilo, porque o HTML apontava para
  classes que não existiam nem no CSS nem no mock. O defeito não aparece em typecheck nem em teste.
- **CSS:** `client/src/public-input.css` (o CSS do mock, verbatim, depois das diretivas do
  Tailwind) → compilado para `client/public/css/public.css` pelo **Tailwind CLI**. A página linka
  `/css/public.css`, nome FIXO: o bundle do Vite tem hash e não serve para o servidor.
- **Textos:** `core/src/i18n/pt.ts` + `en.ts`. `pt.ts` exporta `type Dict = typeof pt`; `en.ts` é
  `const en: Dict`. Chave faltando **quebra a compilação** — que é a trava exigida por `idiomas.md`.
  Nenhum literal de tela dentro do template.
- **Registro da rota:** `app.use("/", homeRouter)` em TODOS os ambientes. Em produção, **depois**
...
- **Dois idiomas no HTML da primeira resposta:** `<html lang>` e `og:locale` do idioma da página, `hreflang` recíproco quando a página tem par no outro idioma, e o `sitemap.xml` com as URLs dos dois idiomas.

## Idiomas — escola bilíngue PT + EN (decisão do operador, Set 2026)

> **A especificação (quem vê o quê, preço fora do Brasil, imposto internacional, o que ainda nasce com idioma, pendências) mora em [`docs/idiomas.md`](docs/idiomas.md).**
> **GATILHO (mecânico):** antes do primeiro write/edit que toque o campo `language`, `User.preferredLanguage`, o dicionário de textos, rota ou template **público** do servidor, `sitemap.xml`/`robots.txt`, ou o formulário de curso/trilha do admin → **ler `idiomas.md`**.
> Aqui ficam **só as travas que produzem um diff ERRADO se ignoradas.**

- **UM ENDEREÇO POR IDIOMA — nunca trocar o idioma servido num mesmo endereço.** Português sem prefixo (os endereços atuais não mudam), inglês sob `/en`. O idioma do navegador **só escolhe para onde levar a primeira visita à raiz**; link direto nunca é redirecionado. **Por quê:** o Googlebot visita principalmente dos EUA e **sem** `Accept-Language` — página que troca de idioma pelo cabeçalho tem uma versão indexada, e a outra some da busca sem erro nenhum.
- **Idioma é do CONTEÚDO, não só da interface.** `Course.language` e `LearningPlan.language` (enum do Prisma), obrigatórios na criação; `Module` e `Lesson` **herdam** do curso, sem coluna própria. **Curso em inglês é OUTRO curso** (vídeos e slug próprios), nunca uma tradução do mesmo registro.
- **Leitura pública filtra por idioma do mesmo jeito que filtra por status** — catálogo, busca, lista de trilhas, `sitemap.xml`. Quem entra em inglês vê só conteúdo em inglês. **Mas SÓ as listas de DESCOBERTA filtram** *(decisão do operador, 24/09/2026 — "como no LinkedIn")*: o que é **do aluno** — trilhas salvas, cursos iniciados, o "continue estudando" — aparece **nos dois idiomas**, qualquer que seja o idioma do app, e salvar ou copiar trilha do outro idioma é permitido. O link direto para um curso ou trilha também não filtra. **Por que é trava:** filtrar "minhas trilhas" pelo idioma parece coerente e esconde do aluno bilíngue o que ele já salvou. Nas rotas, o filtro sai de `idiomaDaLista()` (`server/src/lib/language.ts`) — só nas listas. **Curso publicado com zero aulas é caso real** (os primeiros cursos em inglês nascem assim): a página mostra "0 aulas" normalmente, por decisão do operador, e não pode quebrar com a lista vazia.
- **Botão de assinar nas páginas em inglês só liga com ≥ 1 aula publicada em inglês** *(decisão do operador)*. A condição é **derivada do banco pela cadeia inteira** (aula → módulo → curso EN, todos publicados), **nunca** um interruptor manual. É **regra de exibição, não de acesso**: o checkout não recusa ninguém por ela.
- **IDIOMA É FILTRO, NUNCA PORTÃO** *(decisão do operador, 14/09/2026 — modelo LinkedIn Learning)*: **uma assinatura dá acesso aos cursos dos DOIS idiomas**; o idioma escolhido só decide o que aparece. `temAcessoAtivo()` **não lê idioma** e a `Subscription` **não tem** coluna de idioma. **Por que é trava:** "proteger" o acesso por idioma parece arrumação e tranca fora quem pagou — foi proposto e desfeito no mesmo dia, e um teste da Fase 4 prova o acesso cruzado. **Moeda e idioma são independentes** (a moeda sai do cartão).
- **No máximo 2 idiomas** *(decisão do operador)*: o enum tem dois valores, e um terceiro é decisão nova dele, não "mais um valor". Endereços em inglês usam segmentos em inglês: `/en/courses`, `/en/course/:slug`, `/en/learning-path/:slug`, `/en/certificate/:publicId`.
- **Escrita confere o idioma do que referencia:** trilha só aceita curso/aula do **mesmo idioma** (400 `LanguageMismatch`), e quem recusa é o **servidor**. **O idioma de curso e trilha só troca enquanto é RASCUNHO** *(operador, 24/09)*: publicado → 409 `LanguageLocked`; em rascunho mas já numa trilha do outro idioma → 409 `LanguageInUse`. É a mesma família de *"Escrita também confere status do que REFERENCIA"* (Server) — a tela esconder a opção não é defesa.
- **Texto visível não é literal no código — sai do dicionário, e o dicionário é UM** para os templates do servidor e para o React. Dois dicionários divergem, e o que diverge é o que ninguém está olhando. Tradução faltando tem que quebrar a **compilação**, não aparecer em produção. Valor de enum (`Level`, `Layer`) continua código; o **rótulo** sai do dicionário. O **Bloco I** da Fase 3 cria o dicionário e migra as telas existentes — por isso ele precede o bloco *Superfície pública indexável*.
- **Moeda:** pelo país do cartão, nunca pelo idioma — trava em *Membership Gating*.
...
- **Language is a FILTER, never an access condition (operator decision, Sep 2026):** one subscription grants the courses of both languages. `temAcessoAtivo(userId)` does not read language; `Subscription` has no language column. See *Idiomas*.
...
- **TRAVA — a MOEDA segue o país do CARTÃO, nunca o idioma do site** *(decisão do operador, Set 2026)*. Cartão do Brasil → R$; cartão de qualquer outro país → US$. Derivar a moeda do idioma — ou do prefixo `/en` — abre arbitragem: o estrangeiro troca o site para português e paga em real. Preços, a questão *preço mostrado × cobrado* e o imposto internacional: `billing.md`.
...
- **Rate-limit de login: o IP vem do `x-real-ip`, e SÓ dele** *(resolvido em 23/09/2026, por medição em produção)*. `advanced.ipAddress.ipAddressHeaders: ["x-real-ip"]` em `server/src/lib/auth.ts`. `[FATO — rota temporária de diagnóstico, já removida: a borda da Railway SOBRESCREVE o `x-real-ip` (valor forjado ignorado), e ele é o IP público real de quem acessa, conferido contra serviço externo. O `x-forwarded-for` chega com DOIS elementos — o cliente e um proxy interno que muda a cada requisição.]`
  **Por que a linha não pode sair** (é a que alguém "simplifica"): o Better Auth 1.7.5 **recusa cadeia com mais de um elemento**, então com o default (`x-forwarded-for`) o IP não resolve e o limitador cai num **balde único** — a regra padrão do login é **3 tentativas em 10 s**, e ela passa a valer para a escola inteira: três erros de qualquer pessoa travam o login de todos, admin incluso. Guardado por `server/src/test/auth-ip.test.ts` (verificado por mutação).
  **Nunca** usar `fastly-client-ip`, `true-client-ip` nem `cf-connecting-ip`: medido que repassam o que o visitante escreve.
  **Sintoma de regressão**, e ele é barulhento: o log da Railway volta a mostrar *"Rate limiting could not determine a client IP and is falling back to a single shared per-path bucket"*.
  **A fonte que estava errada:** o suporte da Railway disse ao operador, em 2026, que com a CDN o `x-real-ip` trazia o IP da Fastly. Medido hoje, nenhum IP da cadeia é da Fastly. **Documentação e suporte envelhecem; a medição em produção é o que decide.**
  **`app.set('trust proxy')` NÃO configura isto** — conserta o `req.ip` do Express, não o do Better Auth. *Gatilho de reabertura: trocar de hospedagem ou pôr uma CDN na frente da Railway — os dois mudam quem escreve o `x-real-ip`.*
  Limite adicional **por conta** (independente de IP) só se o Better Auth oferecer pronto; código nosso no caminho de login segue **adiado** por decisão do operador.
- Claude code review on PRs: pendente.
- Subagents in `.claude/agents/`; skills pinned in `skills-lock.json` — **que passou a existir de verdade em Set 2026** (a linha antiga descrevia um arquivo que o repo nunca teve). Hoje ele fixa as **7 skills do Neon** (`neondatabase/agent-skills`), com hash por skill. Conteúdo em `.agents/skills/` (244 KB de markdown, sem executável); `.claude/skills/` são **symlinks** para lá e são versionados por exceção no `.gitignore` — sem eles, um clone novo traz o conteúdo mas não a ligação, e as skills nunca carregam, sem erro e sem aviso. Ferramental de agente **não versionado** (plugin Modern Web Guidance + `chrome-devtools` MCP) está inventariado em [`docs/agent-tooling.md`](docs/agent-tooling.md) — máquina nova não tem nenhum dos dois, a restauração está lá.
...
*(i) **COLD START: MEDIDO, e o gatilho fecha com número em vez de precaução.** `[FATO — Set 2026, `GET /api/courses` em produção, compute comprovadamente suspenso: **COLD 1,544 s** · warm 0,394 / 0,367 / 0,373 s ⇒ **custo de acordar ≈ 1,2 s**. Autosuspend confirmado em **~5 min** de ociosidade.]` Uma medição anterior via `psql` estimou ~820 ms e **subestimava**: ponta a ponta o Prisma ainda refaz a conexão pelo pooler. **Leitura correta do resultado:** mesmo frio o site responde em 1,54 s, dentro da meta de **< 3 s** — mas isso é a **chamada de API**, não a página completa, então **~1,2 s do orçamento já está comprometido** com acordar o banco na primeira visita após ociosidade. Some HTML/CSS/JS/imagens em cima antes de concluir que sobra folga. **Enquanto o tráfego for zero, quase todo primeiro acesso do dia paga isso**; o problema se dissolve sozinho conforme houver alunos, porque o compute deixa de ficar ocioso. **GATILHO (agora específico):** volta à mesa se uma medição de **página completa** estourar os 3 s por causa do cold start, **ou** se houver reclamação real de aluno. **Não é gatilho:** preço, que se resolve trocando de plano.*
*(j) **"Configurar para nunca dormir" — PROPOSTO e INDISPONÍVEL, não rejeitado por gosto** *(Set 2026)*. `[FATO — doc do Neon: no **Free** o timeout de 5 min é **fixo**; **Launch** permite desligar o scale-to-zero; **Scale** permite de 1 min a sempre-ligado.]` Então hoje não é uma opção, é uma mudança de plano. **E a conta não fecharia mesmo assim:** o Free inclui **100 CU-hours/projeto** e compute sempre ligado consome **182,5** (730 h × 0,25 CU) — quase o dobro; no Launch, a **$0,106/CU-hour**, dá **~US$ 19/mês**. Isto é, ~US$ 19 para eliminar 1,2 s na primeira visita de um período ocioso, devolvendo **metade** do que a saída do Supabase economizou, num site com zero tráfego. **E o problema anda na direção contrária do custo:** mais alunos ⇒ compute raramente ocioso ⇒ menos cold start. Pagar agora é pagar mais caro quando ele importa menos. **GATILHO:** reavaliar **quando o plano pago já tiver sido contratado por outro motivo** (a janela de PITR da Fase 7) — aí a opção passa a existir sem custo de plano adicional e a decisão vira só compute.*

*Atualizado Set 2026 (16) — **A ESCOLA PASSA A SER BILÍNGUE (PT + EN) DESDE O LANÇAMENTO, revertendo o "EN removido da escola, PT pra sempre" de Jun 2026. Decisão do operador, 14/09/2026.***
*(a) **O QUE FOI DECIDIDO, pelo operador, em duas rodadas de perguntas:** estrutura bilíngue como a da Udemy, com o inglês **ligado no lançamento mesmo sem nenhum curso em inglês** ("ver a escola nascer em inglês e o Google já indexar") · idioma escolhido pelo **navegador** · quem entra em inglês vê **só cursos em inglês** · **campo Idioma na criação** do curso e da trilha · **trilha segue o idioma** · **moeda pelo país do cartão**, US$ 30/mês fora do Brasil, anual pelo mesmo cálculo do real · depois, **canal do YouTube separado em inglês** com os cursos regravados — *"dois trabalhos, mas duas fontes de renda"*. Especificação completa e o que ficou pendente: `docs/idiomas.md`.*
*(b) **Por que a estrutura entra AGORA, sem curso em inglês:** a Fase 3 reconstrói as páginas públicas no servidor e a Fase 4 cria os preços. Nascendo bilíngues, custam um bloco (o **Bloco I**, antes da *Superfície pública*); com o idioma depois, as duas seriam refeitas. É o mesmo raciocínio do escape na renderização: retrofit é o que deixa furo.*
*(c) **Três travas nasceram da decisão, e cada uma evita um dia ruim que o operador não veria:** **um endereço por idioma** (senão o Googlebot, que visita dos EUA sem cabeçalho de idioma, indexa só uma versão e a outra some da busca sem erro) · **moeda pelo cartão e não pelo idioma** (senão o estrangeiro troca para português e paga em real) · **o servidor recusa trilha com item de outro idioma** (a tela esconder não é defesa — mesma família do status do que referencia).*
*(d) **Duas sugestões externas (Gemini, 14/09) NÃO entraram, com o motivo.** `react-i18next` + rotas `/pt/cursos` no `App.tsx`: as páginas públicas **saem do React** na Fase 3, então o idioma delas não pode morar no React Router, e o dicionário tem que servir servidor e React — a biblioteca, se houver, é decisão do plano do Bloco I. *"A Stripe Tax cuida de tudo"*: `[FATO — docs da Stripe via context7]` ela calcula e cobra, mas **registro e declaração no governo estrangeiro ficam com o vendedor**.*
*(e) **Imposto internacional fica PENDENTE e não é detalhe:** na União Europeia, vendedor de fora deve IVA **desde a primeira venda**, sem mínimo — **isto já vale hoje para aluno de Portugal**, com ou sem inglês. A escolha (Stripe Tax + papelada com o contador × Stripe Managed Payments, que tira a papelada mas exige a página de pagamento da Stripe e colide com o Payment Element embutido) é da **Fase 4, antes da primeira venda fora do Brasil**.*
*(f) **O que NÃO mudou:** a meta de receita e a derivação dela (`strategy.md` §6) seguem **só em reais** — o assinante em dólar ainda não tem conta; `temAcessoAtivo()` não lê idioma; slug continua permanente, agora por curso em cada idioma.*
*(g) **GATILHO DE REABERTURA** *(proposto pelo agente, a confirmar pelo operador)*: a decisão volta à mesa se manter o inglês passar a **atrasar o português de forma medida** — um bloco de build ou um curso PT atrasado por tradução — **ou** se, **12 meses após o lançamento, não houver nenhum curso em inglês publicado**, caso em que a estrutura custa manutenção sem render nada. **Não é gatilho:** poucos alunos em inglês no começo, que o próprio operador já previu.*
*(h) **COMPLEMENTO, mesma data (2ª rodada de respostas do operador) — substitui o (g) e corrige uma proposta do agente.** **Sem gatilho de reabertura:** *"vai sendo construído em paralelo"* — estrutura permanente, o (g) fica registrado como recusado. **O acesso é por idioma:** cada assinatura dá acesso só aos cursos do idioma do cadastro (*"quem acessa do Brasil não vai entender em inglês, e o inverso o mesmo"*). Isso **corrige** a trava escrita horas antes nesta mesma seção ("o gate de acesso não lê idioma"), que era proposta do agente e nunca foi aprovada. **Custo aceito, registrado:** a checagem de idioma entra no ponto mais arriscado do produto (`temAcessoAtivo()`, Fase 4) e cria uma causa nova de *assinante pagante trancado para fora*; por isso ela mora dentro da função única e ganha casos próprios na matriz de testes. **Teto: 15 cursos por idioma, no máximo 2 idiomas.** Endereços `/en/courses`; anual US$ 299; catálogo em inglês sem curso mostra um mock "Excel + AI" que não é linha de banco.*
*(i) **COMPLEMENTO, mesma data (3ª rodada) — substitui dois pontos do (h).** **IDIOMA É FILTRO, NÃO PORTÃO**, no modelo do LinkedIn Learning: assina em inglês e vê os cursos em inglês; troca o idioma e estuda os do outro **na mesma assinatura**. Volta a valer "`temAcessoAtivo()` não lê idioma", agora como **decisão do operador** e não mais como proposta do agente; a `Subscription` não ganha idioma, e os casos 16–18 da Fase 4 viram **um** caso de acesso cruzado. **Por que isto fica escrito como trava:** o acesso por idioma foi decidido e desfeito no mesmo dia; sem a trava, um agente futuro o "redescobre" como proteção. **O mock sai:** o operador cadastra um ou dois cursos em inglês ainda sem aulas, e curso publicado sem aula vira estado a tratar. **Tagline EN:** "Become a data expert in the AI era."*
*(j) **COMPLEMENTO, mesma data (4ª rodada).** Endereços `/en/learning-path/:slug` e `/en/certificate/:publicId`. Curso sem aulas mostra **"0 aulas"** normalmente, sem estado especial (corrige o "estado a tratar" do (i)). **O botão de assinar nas páginas em inglês liga com a primeira aula em inglês**, derivado do banco pela cadeia de status. **Por que é regra de exibição e não trava de checkout:** a assinatura não tem idioma, então quem assina pela página em português está certo, e recusar no servidor seria bloquear um pagamento legítimo.*

*Atualizado Set 2026 (17) — **A HOME PÚBLICA SAIU DO REACT E ESTÁ NO AR EM HTML DE SERVIDOR, BILÍNGUE. Decisão do operador; mock aprovado antes.***
*(a) **O que existe agora:** `/` e `/en` servidas pelo Express, sem React e sem hidratação, com `<html lang>`, `canonical`, `hreflang` recíproco, Open Graph e JSON-LD no HTML da primeira resposta. `escapeHtml()`/`jsonLd()` nasceram junto, com teste unitário, e o teste de servidor bate no app real (`/` e `/en` respondem, `/en` sem curso não quebra).*
*(b) **A regra que o erro produziu: mock aprovado se TRANSPÕE, não se reinterpreta.** A primeira implementação reescreveu a marcação com classes que não existiam em lugar nenhum; o typecheck passou, os testes passaram (contra dublê de Prisma) e metade da página ficou sem estilo. Transposição mecânica — mesma marcação, mesmas classes, só os dados saindo — resolveu. Registrado porque o modo de falha é invisível para os gates.*
*(c) **Teste que mockava o Prisma foi REESCRITO.** Ele montava um Express vazio e trocava o banco por dublê: não provava que a rota estava registrada, nem que o portão "Em breve" continuava na frente. É o defeito nomeado em Testing ("teste que não pode falhar") e foi substituído por supertest contra o app real.*
*(d) **Schema sem migration = suíte vermelha.** A mesma implementação acrescentou `Course.language` no `schema.prisma` sem gerar a migration; o banco não tinha a coluna e um teste que passava há meses quebrou. Revertido. **Quando `language` voltar, volta como enum do Prisma** (o banco recusa valor inválido), com migration na mesma mudança — e `LearningPlan` junto.*
*(e) **Dicionário bilíngue em `core/src/i18n/`**, tipado para que tradução faltando quebre a compilação. As ~109 chaves do inglês estão vazias: a home `/en` responde e não quebra, mas o texto em inglês ainda não foi escrito.*
*(f) **Decisões de produto que a home fixou** (detalhe em `content.md`): hero sem botão de assinar · um cartão de preço, com o anual oferecido no checkout · reembolso de 7 dias por lei (CDC art. 49) · pagamento em cartão E Pix recorrente · depoimentos reais com nome completo e regra de remoção a pedido · sem oferta de fundador e sem escassez fabricada · pode prometer o que o aprendizado entrega, nunca emprego ou salário.*
*(g) **GATILHO DE REABERTURA:** a fronteira volta à mesa se a superfície pública passar a exigir estado compartilhado de verdade entre páginas (busca facetada, carrinho, personalização de logado) — é o mesmo gatilho do Next.js rejeitado em Ago 2026.*
