...
- **Leitura pública filtra por idioma do mesmo jeito que filtra por status** — catálogo, busca, lista de trilhas, `sitemap.xml`. Quem entra em inglês vê só conteúdo em inglês, **inclusive enquanto não existir nenhum** — nesse período o catálogo mostra um **mock "Excel + AI" que é conteúdo fixo do template, NUNCA uma linha de `Course`** (linha no banco vazaria para busca, sitemap e JSON-LD como curso à venda).
- **O ACESSO É POR IDIOMA** *(decisão do operador, 14/09/2026)*: **cada assinatura dá acesso só aos cursos do idioma do cadastro.** O idioma mora na `Subscription`, nunca no `User`, e a checagem mora **DENTRO de `temAcessoAtivo()`**, nunca inline na rota — a fonte única de acesso continua única. **Não mexe na regra do gate** (status vivo OU período pago): é uma condição a mais, não uma reabertura. Construído na Fase 4. **Moeda e idioma são independentes** (a moeda sai do cartão).
...
- **Access is per LANGUAGE (operator decision, Sep 2026):** a subscription grants only the courses of its signup language. The language lives on `Subscription`, and the check lives **inside** `temAcessoAtivo()` — which therefore needs the course's language, not just `userId`. Never a separate language check inline in a route. See *Idiomas*.
