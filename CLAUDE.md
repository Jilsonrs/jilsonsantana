...
*Atualizado Set 2026 (18) — **O BUNNY ENTRA NA ESCOLA: conta, Storage e CDN das imagens configurados no painel pelo operador (25/09/2026). Nenhum código e nenhuma variável de ambiente ainda.** Estado completo, fatos da doc e pendências: `docs/bunny.md`.*
*(a) **Conta:** e-mail do admin, 2FA por app, pessoa física ("Jilson Santana", sem CNPJ), **pré-paga com recarga automática**. **Fato da doc que torna a recarga crítica:** saldo zerado ⇒ conta suspensa e dados do Storage **podem ser apagados** (a doc diverge no prazo; vale o pior caso).*
*(b) **Storage Zone `jilsonsantana-storage`: Standard (HDD), principal São Paulo, réplica New York, S3 desligado.** Tier, S3 e réplica são **IRREVERSÍVEIS** (fato da doc: réplica só se acrescenta; a saída é recriar a zona). A zona de dev **não** foi criada.*
*(c) **Imagens em `img.jilsonsantana.com`** (decisão 4 do `bunny.md` §6, fechada): Pull Zone com SSL, *Allowed referrers* no domínio, **Block direct URL DESLIGADO de propósito** (a prévia do WhatsApp/LinkedIn e o e-mail chegam sem referrer), cache de 1 mês, limite de 100 GB/mês, Shield e Optimizer desligados. Testada de ponta a ponta.*
*(d) **Consequência que o bloco de envio de arquivo herda:** a CDN não vê mudança na origem, então **toda** imagem enviada ganha nome novo a cada envio; e apagar do Storage **não** tira a foto da CDN. A purga na exclusão de conta (LGPD) é **decisão pendente** do operador.*
*(e) **GATILHOS DE REABERTURA:** réplica em Frankfurt **quando houver alunos fora do Brasil** · nome da conta **quando a empresa for aberta** · tier/S3/réplica **só com uma zona nova**, porque não mudam na zona existente.*
