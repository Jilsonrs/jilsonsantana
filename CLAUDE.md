...
- **`prisma migrate dev` NÃO FUNCIONA NESTE REPO — migration nova se escreve à MÃO + `migrate deploy`.** `[FATO — medido em Set 2026: falha com P1014, "The underlying table for model `public._prisma_migrations` does not exist".]` **Causa:** o `migrate dev` valida o histórico replicando-o num **shadow database**, e a migration `20260824214838_rls_prisma_migrations_table` faz `ALTER TABLE public._prisma_migrations` — tabela que o shadow **não tem**. Ele morre antes de gerar qualquer SQL.
  **Por que isto é regra e não nota de rodapé:** o comando falha apontando para a migration de RLS, e a "correção" óbvia — apagar ou editar aquela migration — é proibida (`CLAUDE.md` → Database) **e** quebraria o checksum nos dois bancos que já a aplicaram. O caminho certo é o de baixo.
  **O procedimento:** criar `prisma/migrations/<YYYYMMDDHHMMSS>_<snake_case>/migration.sql` à mão (com o `ENABLE ROW LEVEL SECURITY` de toda tabela nova na MESMA migration) → `npx prisma migrate deploy` → `npx prisma generate` → **provar que não há drift** com `npx prisma migrate diff --from-schema-datasource prisma/schema.prisma --to-schema-datamodel prisma/schema.prisma` (o esperado é *No difference detected*) → rodar a suíte de servidor, que dá `migrate reset` num banco limpo e **prova que a migration replica do zero**. `migrate deploy` e `migrate reset` não usam shadow, então os dois funcionam.
  *Gatilho de reabertura: só volta a funcionar se a migration de RLS virar condicional (`IF EXISTS`) — o que exige `migrate resolve` no dev E na produção, porque muda o checksum de uma migration já aplicada. **Decisão do operador**, não do agente.*
