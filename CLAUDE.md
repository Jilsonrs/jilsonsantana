...
*Atualizado Set 2026 (19) — **O STREAM ESTÁ CONFIGURADO: a biblioteca de aulas `jilsonsantana-stream` existe, está protegida e foi testada; o vídeo vai entrar pelo admin. Decisões do operador, 25/09/2026. Nenhum código e nenhuma variável ainda.** Detalhe: `docs/bunny.md` §3 e §7.1.*
*(a) **Decisão 3 fechada: upload pelo admin**, e as coleções também são criadas pelo admin, nunca à mão no painel. É código novo na Fase 3 (alto risco), e o operador já fixou as regras dele: arquivo original byte a byte e retomável · trocar o vídeo **sem** recriar a aula · progresso preso à **AULA**, nunca ao ID do vídeo · legenda `.vtt` casada pelo nome do arquivo.*
*(b) **Decisão 5 fechada: sem Enterprise DRM** (US$ 99/mês por biblioteca, e só se desliga pelo suporte); **MediaCage Basic, grátis, ligado**.*
*(c) **Segurança da biblioteca de aulas:** *Allowed domains* no domínio da escola (fecha o PENDENTE DE VERIFICAÇÃO da Fase 3), block direct URL, embed token e CDN token ligados, direct play desligado, sem trava por IP. **Testado:** toca no painel e é bloqueado numa aba anônima. Chromecast ficou de fora, a confirmar pelo operador.*
*(d) **Keep Original desligado:** o original **não** fica no Bunny. Recodificar é reenviar a partir das cópias do operador (HD externo + Descript).*
*(e) **Trava de custo para o build: o código NUNCA pede transcrição nem liga o Enterprise DRM.** Fato da doc: pedir transcrição cobra mesmo com a opção desligada na biblioteca.*
*(f) **GATILHOS DE REABERTURA:** Chromecast **quando o operador confirmar** · região São Paulo e réplica **só com uma biblioteca nova**, porque réplica não se remove · *(propostos pelo agente, a confirmar pelo operador)* Enterprise DRM **se houver vazamento de aula medido e receita que pague a mensalidade** · Keep Original **se as cópias do operador deixarem de existir fora do Bunny**.*
