...
- **Ainda falta no bloco Gates: rate-limit de login.** Bloco próprio; toca `server/src/lib/auth.ts`, logo **dispara o gate do context7** (`/better-auth/better-auth`). **O passo 1 é MEDIR a borda, nunca codar** — o protocolo executável (rota temporária, os três testes, o critério de aprovação) é checkbox em `implementation-plan.md` → Fase 3, Bloco 0. Duas armadilhas ficam aqui porque são o que faz alguém "simplificar" errado:
  ...
  2. **A premissa "ele lê o primeiro elemento do XFF" está CONFIRMADA para a versão instalada — e a documentação oficial descreve outra coisa.** `[FATO — medido em Set 2026 no código da 1.6.20 deste repo: `better-auth/dist/utils/get-request-ip.mjs:12` faz `value.split(",")[0].trim()`.]` Ou seja: cadeia separada por vírgula **é** confiada, e o primeiro elemento vence. **Se a borda da Railway ANEXAR em vez de sobrescrever, o atacante escolhe o próprio balde** e o brute-force é ilimitado.
     **A armadilha REAL descoberta aqui não é a premissa, é a FONTE:** a documentação atual do Better Auth (via context7, Set 2026) diz que ele *"does not trust comma-separated forwarded IP chains"* e oferece `trustedProxies` — **as duas coisas descrevem uma versão MAIS NOVA**. `trustedProxies` **não existe** na 1.6.20 (medido: nenhuma ocorrência em `dist/`). Documentação de biblioteca descreve o `main`, não a versão que você instalou; num repo com versão fixada, **ler o código instalado é a única verificação que vale**.
     *Caminho de saída registrado: subir o Better Auth traria o default seguro + `trustedProxies`. É dependência nova, logo decisão do operador — e mesmo assim não dispensa o Passo 1, porque ninguém sabe o que a borda da Railway faz sem medir.*
  3. **`customRules` EXISTE na 1.6.20** `[FATO — `dist/api/rate-limiter/index.mjs`]`: dá para apertar `/sign-in/email` sozinho (ex.: 3 tentativas em 10 s) sem mexer no resto. **Mas não conserta o buraco** — limite indexado por IP falsificável continua falsificável. Serve como segunda camada depois do Passo 1, nunca no lugar dele.
