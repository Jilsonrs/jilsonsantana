...
- **Leitura pública filtra por idioma do mesmo jeito que filtra por status** — catálogo, busca, lista de trilhas, `sitemap.xml`. Quem entra em inglês vê só conteúdo em inglês. **Curso publicado com zero aulas é caso real** (os primeiros cursos em inglês nascem assim): a página mostra "0 aulas" normalmente, por decisão do operador, e não pode quebrar com a lista vazia.
...
- **Escrita confere o idioma do que referencia:** trilha só aceita curso/aula do **mesmo idioma**, e quem recusa é o **servidor**. É a mesma família de *"Escrita também confere status do que REFERENCIA"* (Server) — a tela esconder a opção não é defesa.
