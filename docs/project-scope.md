...
- Single membership subscription via **Stripe Payments + Stripe Billing, with the Payment Element embedded in our own checkout page** — **Mensal R$99,90 (sem fidelidade) + Anual ~R$995 (~17% off); sem free trial; sem conteúdo grátis na escola.** **Outside Brazil: US$ 30/month + US$ 299/year (same math); currency follows the card's country, never the site language** (operator decision, Sep 2026). **Each subscription grants only the courses of its signup language.** Access gated by active subscription.
- **Bilingual platform (PT + EN) from launch** (operator decision, 14 Sep 2026 — spec in `idiomas.md`): the browser language routes the first visit; one URL per language (PT unprefixed, EN under `/en`); every course and trilha is created in one language, and a visitor in English sees only English courses — even while there are none. English courses are separate, re-recorded courses. Catalog cap: **15 courses per language, 2 languages max**; while there is no English course, the English catalog shows an "Excel + AI" mock.
