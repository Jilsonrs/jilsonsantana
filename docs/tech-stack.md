...
- **Neon (PostgreSQL serverless)** — projeto `falling-snow-79489296` (`jilsonsantana-prod`), região **aws-us-east-2**, **PostgreSQL 18.6**. Plano **Free** hoje; custo **US$ 0**. *(Migrado do Supabase em Set 2026 — o porquê, a verificação e os gatilhos estão no changelog do `CLAUDE.md`, entrada (15).)*
- **UM projeto, DOIS branches — não dois projetos.** `production` (endpoint `ep-still-breeze-aebrui0f`) = **produção**, alcançável só pelo Railway · `dev` (endpoint `ep-lingering-morning-aehqd81z`) = **dev / a escola**, nunca apagado. O banco de **teste** é local (ver *Testing*). **Isto é o que derrubou o custo:** no Supabase o 2º ambiente era um 2º projeto (US$ 10/mês no plano pago); aqui é um branch copy-on-write, que custa ~zero e nasce com os dados do pai.
- **Branch é isolado em DADOS, não em CREDENCIAL.** Branch novo herda a senha da role do pai — rotacionar antes de usar. Regra e o incidente que a produziu: `CLAUDE.md` → Database & Migrations.
- **REGIÃO us-east-2, e não recriar projeto por causa disso.** A latência extra pro Brasil (~150 ms por query) **não é o gargalo**: o **vídeo** domina o tempo do aluno e vem do **Bunny com PoP local**, e a meta de carregamento é **< 3 s** — 0,15 s é **5%** desse orçamento. Consultas em **paralelo** não somam latência; o que custa é consulta em **cascata**, e isso é **decisão de código, não de região**. *Gatilho de reabertura: reclamação real de lentidão de aluno, **OU** outro motivo que exija recriar o projeto.*
- **O que come o orçamento de 3 s não é a região — é o COLD START.** `[FATO — Set 2026, medido em produção: COLD **1,544 s**, warm **~0,377 s** ⇒ acordar o compute ≈ **1,2 s**; autosuspend em ~5 min de ociosidade.]` Ou seja: **~8× o custo da região.** Otimizar região ou cascata de query antes de considerar o cold start é olhar para o lugar errado. O número acima é da **chamada de API**; página completa soma assets em cima. Detalhe e gatilho: `CLAUDE.md` → changelog (15), item (i).
- Accessed **only via Prisma** through `DATABASE_URL` (pooled) / `DIRECT_URL` (direta). As duas **mudam juntas** — ver `CLAUDE.md`.
- **RLS convention:** every `public` table has Row Level Security ENABLED (without policies) in the same migration that creates it. Prisma conecta como a role dona e é o único acessor. **O alvo mudou de fornecedor:** era a Data API do Supabase, agora é a **Data API do Neon** (aba do diálogo *Connect*, hoje desligada). Verificação após cada migration de DDL = **SQL sobre `pg_class.relrowsecurity`**, não o `get_advisors` do MCP do Supabase, que não existe aqui. NOT used: Neon Auth, Data API, Storage.

## Authentication

- **Better Auth** — email/password, **database sessions**, Prisma adapter on the Neon Postgres.
- **Isto NÃO é o auth do fornecedor de banco** (não era o Supabase Auth, não é o Neon Auth). As sessões vivem no **nosso** Postgres, acessadas via Prisma — biblioteca mantida em vez de código artesanal. **Set 2026 mostrou o valor prático disso:** a troca de fornecedor de banco não tocou **uma linha** de autenticação; as tabelas `user`/`session`/`account` migraram como dados comuns.
...
- **Preços = objetos `Price` da Stripe** (um produto "Assinatura", dois prices): Mensal **R$99,90 (sem fidelidade)** + Anual **~R$995 (~17% off)**. **Fora do Brasil (Set 2026): US$ 30/mês + anual pelo mesmo cálculo, moeda pelo país do cartão** — a forma na Stripe (moedas no mesmo `Price` ou prices separados) e a decisão de imposto internacional ficam para a Fase 4 (`billing.md`, `idiomas.md` §5). **Sem free trial, sem conteúdo grátis, sem trava de preço vitalícia.** Troca mensal↔anual = `subscriptions.update` com proração da Stripe (previsualizável antes de mostrar ao aluno).
...
- **Nenhum provedor muda por causa disto.** Railway, Neon, Bunny, Stripe e Resend seguem. Registrado de propósito: *"vamos fazer certo"* é o clima em que se troca infraestrutura que estava boa. Nada se troca sem uma falha nomeável. *(A troca Supabase → Neon em Set 2026 foi decisão do operador, não um caso de "melhorar por melhorar" — está no changelog do `CLAUDE.md`, entrada (15).)*
...
- **Auth / Data API / Storage do fornecedor de banco** (era Supabase, agora Neon) — auth is Better Auth via Prisma; all DB access via Prisma.
